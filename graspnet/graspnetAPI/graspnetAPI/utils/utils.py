...
def generate_views(N, phi=(np.sqrt(5)-1)/2, center=np.zeros(3, dtype=float), R=1):
    ''' Author: chenxi-wang
...
def generate_scene_pointcloud(dataset_root, scene_name, anno_idx, align=False, camera='kinect'):
    ...
    **Output:**
    ...
    '''
    colors = np.array(Image.open(os.path.join(dataset_root, 'scenes', scene_name, camera, 'rgb', '%04d.png'%anno_idx)), dtype=float) / 255.0
...
def get_batch_key_points(centers, Rs, widths):
    ...
    **Output:**
    ...
    depth_base = 0.02
    height = 0.02
    key_points = np.zeros((centers.shape[0],4,3),dtype = float)
...
def batch_key_points_2_tuple(key_points, scores, object_ids, camera):
    ...
    **Output:**
    ...
    tuples = np.concatenate([centers, opens, heights, scores[:, np.newaxis], object_ids[:, np.newaxis]], axis=-1).astype(float)
    return tuples
...
def batch_key_point_2_rotation(centers_xyz, open_points_xyz, upper_points_xyz):
    ...
    **Output:**
    ...
    x_axis = np.hstack((np.zeros((num, 1)), np.zeros((num, 1)), np.ones((num, 1)))).astype(float).reshape(-1, 3, 1)
