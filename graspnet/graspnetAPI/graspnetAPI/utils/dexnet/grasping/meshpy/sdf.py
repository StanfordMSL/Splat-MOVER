...
class Sdf3D(Sdf):
    ...
    def transform_dense(self, delta_T, detailed=False):
        ...
        if self.pts_ is None:
            ...
            self.pts_ = np.c_[x_ind.flatten().T, np.c_[y_ind.flatten().T, z_ind.flatten().T]].astype(float)
    ...
    def transform_pt_obj_to_grid(self, x_sdf, direction=False):
        ...
        if direction:
            points_sdf = NormalCloud(x_sdf.astype(float), frame='world')
        else:
            points_sdf = PointCloud(x_sdf.astype(float), frame='world')
    ...
    def transform_pt_grid_to_obj(self, x_grid, direction=False):
        ...
        if direction:
            points_grid = NormalCloud(x_grid.astype(float), frame='grid')
        else:
            points_grid = PointCloud(x_grid.astype(float), frame='grid')
