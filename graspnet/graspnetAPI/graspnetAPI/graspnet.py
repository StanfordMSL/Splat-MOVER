...
class GraspNet():
    ...
    def loadGraspLabels(self, objIds=None):
        ...
        **Output:**
        ...
        for i in tqdm(objIds, desc='Loading grasping labels...'):
            ...
            graspLabels[i] = (file['points'].astype(float), file['offsets'].astype(float), file['scores'].astype(float))
        return graspLabels
    ...
    def loadScenePointCloud(self, sceneId, camera, annId, align=False, format = 'open3d', use_workspace = False, use_mask = True, use_inpainting = False):
        ...
        **Output:**
        ...
        '''
        colors = self.loadRGB(sceneId = sceneId, camera = camera, annId = annId).astype(float) / 255.0
        ...
        **Output:**
        ...
        if format == '6d':
            ...
            for i, (obj_idx, trans) in enumerate(zip(obj_list, pose_list)):
                ...
                obj_grasp_array = np.hstack([scores, widths, heights, depths, rotations, target_points, object_ids]).astype(float)
