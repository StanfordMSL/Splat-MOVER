...
from sagesplat.data.sagesplat_datamanager import SageSplatDataManager

from nerfstudio.utils.colormaps import ColormapOptions, apply_colormap
from sagesplat.viewer_utils import ViewerUtils
...
@dataclass
class SageSplatModelConfig(SplatfactoModelConfig):
    """Splatfacto Model Config, nerfstudio's implementation of Gaussian Splatting"""

    _target: Type = field(default_factory=lambda: SageSplatModel)
...
class SageSplatModel(SplatfactoModel):
    ...
    def populate_modules(self):
        ...
        # datamanager
        self.datamanager: SageSplatDataManager = self.kwargs["datamanager"]
        ...
        clip_embeds = torch.nn.Parameter(
            torch.rand(num_points, self.clip_embeds_latent_dim)
        )
        ...
        affordance = torch.nn.Parameter(torch.rand(num_points, 1))

        # background affordance rendering (3-channel)
        self.affordance_background = torch.zeros(3, device=self.kwargs["device"])
    ...
    @property
    def affordance(self):
        return self.gauss_params["affordance"]
    ...
    def split_gaussians(self, split_mask, samps):
        ...
        # step 7, sample new affordance
        new_affordance = self.affordance[split_mask].repeat(samps, 1)
        out = {
            "means": new_means,
    ...
    def get_semantic_outputs(self, outputs: Dict[str, torch.Tensor]):
        if not self.training:
            ...
            if "rgb" in outputs.keys():
                # composited affordance
                # threshold
                affordance_threshold = 0.2
                ...
                if self.viewer_utils.has_positives:
                    ...
                    # semantic affordance and CLIP rendering
                    p_i = torch.clip(outputs["similarity"] - 0.5, 0, 1)
                    ...
                    outputs["semantic_affordance"][mask, :] = outputs["rgb"][mask, :]

        return outputs

    def get_outputs(
        self, camera: Cameras, compute_semantics: Optional[bool] = True
    ...
    def get_loss_dict(
        ...
        if batch["affordance"].shape[:-1] != outputs["affordance"].shape[:-1]:
            # torchvision can be slow to import, so we do it lazily.
            import torchvision.transforms.functional as TF
        ...
        if self.step < self.scene_train_max_iter:
            ...
            clip_recon = (
                self.clip_decoder(clip_latent)
                .view(*batch["clip"].shape[:-1], self.clip_embeds_input_dim)
                .float()
            )

        # # reshape latent CLIP embeddings
        clip_latent = clip_latent.view(
            *batch["clip"].shape[:-1], self.clip_embeds_latent_dim
        ).float()
        ...
        if self.step == 0 or self.step >= self.scene_train_max_iter:
            ...
            # Supervision in the Latent Space
            clip_img_loss = (
                self.config.clip_img_loss_weight
                * torch.nn.functional.mse_loss(outputs["clip"], clip_latent_img)
            )
        ...
        if self.step < self.scene_train_max_iter:
            ...
            clip_network_loss = (
                self.config.clip_network_loss_weight
                * torch.nn.functional.mse_loss(clip_recon, batch["clip"])
            )

            # Cosine Similarity
            clip_cosine_sim_loss = (
                self.config.clip_network_cosine_sim_loss_weight
                * (
                    1
                    - torch.nn.functional.cosine_similarity(
            ...
            # Affordance Loss
            affordance_loss = (
                self.config.affordance_loss_weight
                * torch.nn.functional.mse_loss(
                    outputs["affordance"], gt_affordance_img, reduction="mean"
                )
            )
    ...
    def get_outputs_for_camera(
        ...
        outs = self.get_outputs(
            camera.to(self.device), compute_semantics=compute_semantics
        )
