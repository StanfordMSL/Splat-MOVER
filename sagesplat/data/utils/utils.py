...
def apply_pca_colormap_return_proj(
    ...
    if proj_V is None:
        ...
        with torch.no_grad():
            U, S, V = torch.pca_lowrank(image_flat - mean, q=subspace_dim, niter=niter)
        proj_V = V[:, :subspace_dim]
    ...
    colored_image = low_rank.reshape(image.shape[:-1] + (subspace_dim,))
