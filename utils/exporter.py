...
class ExportGaussianSplat(Exporter):
    ...
    @staticmethod
    def write_ply(
        filename: str, count: int, map_to_tensors: OrderedDict[str, np.ndarray]
    ):
        """
        ...
        if not all(
            ...
            and tensor.dtype.kind in ["f", "d"]
            and tensor.size > 0
            for tensor in map_to_tensors.values()
        ):
            raise ValueError(
                "All tensors must be numpy arrays of float type and not empty"
            )
